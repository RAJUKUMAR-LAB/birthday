...
// Instant global gesture listeners across window & document in capture phase
// Any tap, click, key press, input, touch, or scroll unlocks and starts Special Song immediately!
const instantAudioUnlock = () => {
    const mgr = getAudioMgr();
    if (mgr && !mgr.isPlayingSpecial && !mgr.isPlayingBGM) {
        mgr.playSpecialSong();
    }
};
['pointerdown', 'touchstart', 'mousedown', 'keydown', 'scroll', 'click', 'focus'].forEach(evt => {
    window.addEventListener(evt, instantAudioUnlock, { passive: true, capture: true });
    document.addEventListener(evt, instantAudioUnlock, { passive: true, capture: true });
});
...
function initQuizFlow() {
    ...
    if (specialBanner) {
        specialBanner.addEventListener('click', (e) => {
            if (e.target.closest('#quiz-special-play-btn')) return;
            const mgr = getAudioMgr();
            if (mgr) mgr.toggleSpecialSong();
        });
    }

    // Auto-trigger special song on any first user gesture on the front page
    const startSpecialOnFirstGesture = () => {
        const mgr = getAudioMgr();
        if (mgr && quizOverlay && !quizOverlay.classList.contains('hidden')) {
            if (!mgr.isPlayingSpecial && !mgr.isPlayingBGM) {
                mgr.playSpecialSong();
            }
        }
        document.removeEventListener('click', startSpecialOnFirstGesture, true);
        document.removeEventListener('touchstart', startSpecialOnFirstGesture, true);
        document.removeEventListener('keydown', startSpecialOnFirstGesture, true);
    };
    document.addEventListener('click', startSpecialOnFirstGesture, true);
    document.addEventListener('touchstart', startSpecialOnFirstGesture, true);
    document.addEventListener('keydown', startSpecialOnFirstGesture, true);
...
function initHUD() {
    ...
    // Auto-start Special Dedication Song on first user gesture anywhere
    ...
    const startAudioOnFirstInteraction = () => {
        ...
        ['click', 'keydown', 'touchstart', 'pointerdown'].forEach(evt => {
            document.removeEventListener(evt, startAudioOnFirstInteraction, true);
        });
    };
    ['click', 'keydown', 'touchstart', 'pointerdown'].forEach(evt => {
        document.addEventListener(evt, startAudioOnFirstInteraction, { capture: true });
    });
